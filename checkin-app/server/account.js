Accounts.onCreateUser(function(profile, user){
    ...
    profile.cover = getPicture(user.services.facebook.accessToken, user.services.facebook.id).source;
...
var getPicture = function (accessToken, id) {
    try {
    return HTTP.get("https://graph.facebook.com/v2.4/" + id, {
        params: {
            access_token: accessToken,
            fields: ['cover']
        }
